...
public class DebugAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.DEBUG.configKey());
        log.info("[DebugAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.DEBUG, ctx, history, prompt));
    }
}
