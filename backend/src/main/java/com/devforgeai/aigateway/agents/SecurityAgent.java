...
public class SecurityAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.SECURITY.configKey());
        log.info("[SecurityAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.SECURITY, ctx, history, prompt));
    }
}
