...
public class DocumentationAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.DOCUMENTATION.configKey());
        log.info("[DocumentationAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.DOCUMENTATION, ctx, history, prompt));
    }
}
