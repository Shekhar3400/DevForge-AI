...
public class GeneralChatAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.GENERAL.configKey());
        log.info("[GeneralChatAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.GENERAL, ctx, history, prompt));
    }
}
