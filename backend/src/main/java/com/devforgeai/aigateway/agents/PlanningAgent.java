...
public class PlanningAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.PLANNING.configKey());
        log.info("[PlanningAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.PLANNING, ctx, history, prompt));
    }
}
