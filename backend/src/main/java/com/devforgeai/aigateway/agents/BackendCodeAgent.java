...
public class BackendCodeAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.BACKEND.configKey());
        log.info("[BackendCodeAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.BACKEND, ctx, history, prompt));
    }
}
