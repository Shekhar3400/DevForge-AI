...
public class FrontendCodeAgent implements AiAgent {
    ...
    @Override public String respond(String ctx, List<Message> history, String prompt) {
        String model = props.modelFor(IntentType.FRONTEND.configKey());
        log.info("[FrontendCodeAgent] model={}", model);
        return provider.complete(model, promptBuilder.build(IntentType.FRONTEND, ctx, history, prompt));
    }
}
